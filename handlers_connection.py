"""Connection lifecycle for Kubecost Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from kubecost_client import KubecostClient
...
@chat.function(
    "connect_kubecost",
    "Connect Kubecost account via credentials.",
    action_type="write",
    chain_callable=True,
    event="kubecost-connector.connect_kubecost",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_kubecost(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Kubecost Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Kubecost Account",
...
@chat.function(
    "list_connections",
    "List connected Kubecost accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_kubecost",
    "Disconnect Kubecost account.",
    action_type="write",
    chain_callable=True,
    event="kubecost-connector.disconnect_kubecost",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_kubecost(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
