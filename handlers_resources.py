"""Resource handlers for Kubecost Connector."""
...
@chat.function("list_costs", "List costs in Kubecost.", action_type="read", chain_callable=True, event="kubecost-connector.list_costs", effects=["read:costs"], data_model=CostRecordList)
...
@chat.function("get_costrecord", "Get details of one CostRecord in Kubecost.", action_type="read", chain_callable=True, event="kubecost-connector.get_costrecord", effects=["read:costrecord"], data_model=CostRecordRecord)
...
@chat.function("audit_costrecord_health", "Audit health of Kubecost costs and connectivity.", action_type="read", chain_callable=True, event="kubecost-connector.audit_costrecord_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_costrecord_health(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.ok({
            ...
            "summary": f"Kubecost healthy. Sampled {len(items)} costs."
        }, summary=f"Kubecost health check passed with {len(items)} costs.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Kubecost health: {e}")
